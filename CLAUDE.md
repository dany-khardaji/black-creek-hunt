...
- Only comment non-obvious logic. No jargon, no restating the code. Do only one-line comments. This is important enough to go to two lines. Format comments like @main.py And make each comment start with a lowercase unless it's something like a structural header in @main.py then its okay to capitalize first letter.
...
Whenever referencing code in your responses, make every file/line reference a clickable Markdown link using this exact format:

[file.ts:42](relative/path/file.ts#L42)

For ranges:

[file.ts:42-58](relative/path/file.ts#L42-L58)

Always use workspace-relative paths.
Do not use plain `file.ts:42` references.
Do not use heading anchors.
